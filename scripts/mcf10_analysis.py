...
encode_list = pickle.load(open(encode_list_file))
mendillo_list = pickle.load(open(mendillo_list_file))


mendillo_mcf10 = [f for f in mendillo_list if "MCF10" in f or "Mcf10" in f]


encode_mcf10 = [f for f in encode_list if "MCF10" in f or "Mcf10" in f]


all_file_data = mendillo_mcf10 + encode_mcf10
all_files_tuple = []
...
writer = pd.ExcelWriter(os.path.join(data_dir,"mcf10_TSS_coassociation.xlsx"))
